...
public class SimFrame extends JFrame implements ActionListener {
	...
	public JComponent buildSimulationInfo()
	{
		...
		pnMain.setBorder(BorderFactory.createTitledBorder("시뮬레이션 정보"));
		...
		cbxFuntion.addItem("포아송 분포");
		cbxFuntion.addItem("일양 분포");
		...
		pnCenter.add(addItem("시험 기간", txfTPeid));
		pnCenter.add(addItem("시간 간격", txfTime));
		pnCenter.add(addItem("반복횟수", txfRepeat));
		pnCenter.add(addItem("랜덤함수", cbxFuntion));
	...
	public String jFileChooserUtil() {
		...
		JFileChooser chooser = new JFileChooser(this.getClass().getResource("/layout").getPath()); // 디렉토리 설정
		//chooser.setCurrentDirectory(new File("/layout")); // 현재 사용 디렉토리를 지정
		chooser.setAcceptAllFileFilterUsed(true);   // Fileter 모든 파일 적용
		chooser.setDialogTitle("레이아웃 선택"); // 창의 제목
		chooser.setFileSelectionMode(JFileChooser.FILES_AND_DIRECTORIES); // 파일 선택 모드

		FileNameExtensionFilter filter = new FileNameExtensionFilter("xml", "xml"); // filter 확장자 추가
		chooser.setFileFilter(filter); // 파일 필터를 추가
		int returnVal = chooser.showOpenDialog(SimFrame.this); // 열기용 창 오픈
